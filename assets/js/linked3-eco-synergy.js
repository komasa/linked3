/**
 * Linked3 Eco Synergy JS
 * Extracted from: admin/views/dashboard/partials/eco-synergy.php
 * v29.1.0 Step 4: Inline JS extracted to assets/js/linked3-eco-synergy.js
 * Localized via wp_localize_script('linked3-eco-synergy', 'linked3_eco_synergy', {...})
 *   Keys: require_html, ajax_url, content_url, publish_url, api_url, draft_url, nonce
 */

(function(){
    var ajaxUrl = 'window.linked3_eco_synergy.ajax_url';
    var nonce = 'window.linked3_eco_synergy.nonce';
...
'<div style="display:flex;gap:8px;flex-wrap:wrap;">' +
                        ...
                        if (goContent) goContent.addEventListener('click', function(){ location.href = 'window.linked3_eco_synergy.content_url'; });
                        var goPublish = document.getElementById('eco-go-publish');
                        if (goPublish) goPublish.addEventListener('click', function(){ location.href = 'window.linked3_eco_synergy.publish_url'; });
                        // v11.9.1: 生成图片按钮事件绑定
        ...
        fetch(ajaxUrl, {method:'POST', body:fd, credentials:'same-origin'})
            ...
            .then(function(data){
                ...
                if (data.success) {
                    ...
                    if (generatedImageUrls.length > 0) {
                        ...
                        document.getElementById('eco-assemble-btn').addEventListener('click', function() {
                            ...
                            var assembleTimeout = setTimeout(function(){
                                ...
                                st.innerHTML = '<span style="color:#F59E0B;">⏰ 组装超时(120秒), 可能是图片下载慢。</span><br><button class="linked3-eco-btn linked3-eco-btn-sm" style="margin-top:4px;" onclick="document.getElementById(\'eco-assemble-btn\').click();">🔄 重试组装</button> · <a href="' + 'window.linked3_eco_synergy.draft_url' + '" target="_blank" style="font-size:12px;color:#0F172A;">查看草稿列表</a>';
                            }, 120000);
                            // v11.8.0: 根据require_html选择图片插入格式
                            var requireHtml = window.linked3_eco_synergy.require_html;
                    ...
                    result.innerHTML = '<div class="notice notice-error inline"><p>' + escHtml(data.data && data.data.message ? data.data.message : '生成失败') + '</p><p><a href="' + 'window.linked3_eco_synergy.api_url' + '">→ 去API设置页配置图片API</a></p></div>';
                }
            })
            .catch(function(e){
                btn.disabled = false;
