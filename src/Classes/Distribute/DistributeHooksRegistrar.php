...
final class DistributeHooksRegistrar
{
    ...
    static function on_publish($new_status, $old_status, $post): void {
     ...
     */
    static function on_shutdown_distribute(): void {
    ...
    static function ajax_test(): void {
        self::guard();
    ...
    static function ajax_save(): void {
        self::guard();
        $platform = sanitize_text_field($_POST['platform'] ?? '');
        $cfg = self::parse_config($platform);
        $configs = get_option(LINKED3_OPTION_PREFIX . 'distribute_configs', []);
