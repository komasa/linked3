 ...
 *
 * Dimensions (per Linked3_SEO_Config::get('scoring.weights')):
...
use Linked3\Classes\SEO\Schema\Linked3_Schema_Markup;
...
final class Linked3_SEO_Scorecard
{
    /**
     * @param \WP_Post $post
     * @return array{
    ...
    : array {
        ...
        $weights = (array) Linked3_SEO_Config::get('scoring.weights', []);
        $total_weight = 0.0;
    ...
    private function primary_keyword($text) : mixed     {
        $kw = (new \Linked3\Classes\SEO\Keyword\Linked3_Keyword_Extractor())->extract_keywords($text, 1);
        return $kw[0] ?? '';
    }
    ...
    {
        $json = Linked3_Schema_Markup::instance()->for_post($post);
        return $json !== '' ? 1.0 : 0.0;
    }
