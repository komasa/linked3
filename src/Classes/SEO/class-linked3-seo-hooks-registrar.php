...
final class Linked3_SEO_Hooks_Registrar
{
    /**
     * @return void
     */
    ...
    : void {
        ...
        $actions = [
            'linked3_push_retry' => Ajax\Actions\Linked3_Push_Retry_Action::class,
            'linked3_push_now'   => Ajax\Actions\Linked3_Push_Now_Action::class,
            'linked3_seo_score'  => Ajax\Actions\Linked3_SEO_Score_Action::class,
        ];
        ...
        $output_hook = (string) Linked3_SEO_Config::get('schema.output_hook', 'wp_head');
        $output_prio = (int) Linked3_SEO_Config::get('schema.output_priority', 10);
        ...
        if (class_exists(__NAMESPACE__ . '\\Hooks\\Linked3_Indexnow_Save_Post_Hook')) {
            Hooks\Linked3_Indexnow_Save_Post_Hook::register();
        }
        ...
        if (class_exists(__NAMESPACE__ . '\\Linked3_GEO_Enhancer')) {
            Linked3_GEO_Enhancer::init();
        }
    }
    ...
    : void {
        $excluded = (array) Linked3_SEO_Config::get('interlink.excluded_post_types', []);
    ...
    : void {
        ...
        if (!class_exists(__NAMESPACE__ . '\\Schema\\Linked3_Schema_Markup')) {
            return;
        }
        // Don't double-emit if an SEO plugin adapter has taken over.
        $adapter = Adapter\Linked3_SEO_Adapter_Detector::resolve();
        if ($adapter && $adapter->handles_schema()) {
            return;
        }
        $markup = Schema\Linked3_Schema_Markup::instance();
    ...
    public static function filter_the_content($content) : mixed {
        ...
        if (!empty($seo_enhance['external_link_nofollow']) || !empty($seo_enhance['external_link_target_blank'])) {
            if (class_exists(__NAMESPACE__ . '\\Links\\Linked3_External_Link_Processor')) {
                $whitelist = array_filter(array_map('trim', explode("\n", $seo_enhance['external_link_whitelist'])));
                $content = Links\Linked3_External_Link_Processor::process($content, [
        ...
        // 智能内链
        if (!empty($seo_enhance['interlink_enabled']) && class_exists(__NAMESPACE__ . '\\Interlink\\Linked3_Interlink_Builder') && is_singular()) {
            $post_id = get_the_ID();
            if ($post_id) {
                $builder = new Interlink\Linked3_Interlink_Builder();
