...
final class Linked3_Indexnow_Save_Post_Hook
{
    ...
    : void {
        ...
        // breaks the dedup.
        $min_gap = (int) Linked3_SEO_Config::get('push.indexnow.min_gap', 10 * MINUTE_IN_SECONDS);
        $engines = (array) Linked3_SEO_Config::get('push.indexnow.engines', ['indexnow']);
        foreach ($engines as $slug) {
            $slug = sanitize_key((string) $slug);
            if ($slug === '') {
                continue;
            }
            if (Linked3_Push_Log_Repository::count_recent_success($url, $slug, $min_gap) > 0) {
