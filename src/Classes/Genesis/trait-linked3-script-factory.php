...
trait Linked3_Script_Factory_Trait {
    ...
    private function fetch_seed_entry(string $type, $id): ?array {
        ...
        if (class_exists('\Linked3\Classes\Genesis\Linked3_Seed_Unified')) {
            try {
                $entry = \Linked3_Seed_Unified::get($id);
        ...
        if (class_exists('\Linked3\Classes\Genesis\Linked3_Genesis_Seed_CPT')) {
            try {
                $entry = \Linked3_Genesis_Seed_CPT::get_by_seed_id($id);
                if (is_array($entry) && !empty($entry)) {
                    return $entry;
                }
            } catch (\Throwable $e) {}
