...
class Linked3_Genesis_RecommendationEngine
{
    /**
     * 风格库实例
     */
    ...
    public function __construct() {
        $this->styleEngine = new Linked3_Genesis_StyleEngine();
    }
    ...
    private function loadAllStyles(): array
    {
        ...
        if (class_exists('\Linked3\Classes\Genesis\Linked3_Genesis_AtomIndex')) {
            $idx = Linked3_Genesis_AtomIndex::instance();
            $raw = $idx->getStyles();
            if (isset($raw['styles']) && is_array($raw['styles'])) {
                foreach ($raw['styles'] as $sid => $sinfo) {
                    $config = Linked3_Genesis_StyleEngine::load($sid);
                    if (!empty($config)) {
                        $config['id'] = $sid;
                        $styles[] = $config;
                    }
    ...
    public function applyModeFilters(array $styles, array $modeConfig, array $features, string $industry): array {
        return Linked3_Genesis_Recommendation_Scorer::applyModeFilters($styles, $modeConfig, $features, $industry);
    }
    ...
    private function scoreByFeatures(array $styles, array $features, string $mode = 'auto'): array {
        return Linked3_Genesis_Recommendation_Scorer::scoreByFeatures($styles, $features, $mode);
    }
