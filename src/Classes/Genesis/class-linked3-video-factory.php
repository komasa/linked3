 ...
 *   - use Linked3_Script_Factory_Trait 获得共享能力
...
class Linked3_Video_Factory {
    ...
    private function generate_video_group(array $beat, string $style_keywords, int $index): array {
        ...
        if (class_exists('\Linked3\Classes\Genesis\Linked3_Motion_Prompt_Engine')) {
            try {
                if (method_exists('\Linked3\Classes\Genesis\Linked3_Motion_Prompt_Engine', 'generate_video_group')) {
                    $engine_group = \Linked3_Motion_Prompt_Engine::generate_video_group($beat, [
                ...
                // 降级: 仅生成Motion Prompt
                if (method_exists('\Linked3\Classes\Genesis\Linked3_Motion_Prompt_Engine', 'derive_from_emotion')) {
                    $params = \Linked3_Motion_Prompt_Engine::derive_from_emotion($beat['emotion'], $beat['arc_position']);
                    $motion = \Linked3_Motion_Prompt_Engine::generate($params);
                }
            } catch (\Throwable $e) {
                $motion = '';
            }
