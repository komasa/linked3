...
class Linked3_Genesis_SeedDNA
{
     ...
     * 优先委托给 CPT 版本 (Linked3_Genesis_Seed_CPT::listAll)。
     */
    public static function listAll(int $limit = 200): array
    {
        // 优先委托 CPT (如果可用)
        if (class_exists('\Linked3\Classes\Genesis\Linked3_Genesis_Seed_CPT') && method_exists('\Linked3\Classes\Genesis\Linked3_Genesis_Seed_CPT', 'listAll')) {
            return \Linked3_Genesis_Seed_CPT::listAll($limit);
        }
        // 兜底: 旧 option 存储
        $out   = [];
