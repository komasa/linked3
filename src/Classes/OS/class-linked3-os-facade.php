 ...
 *   - 单一入口: 所有V18功能通过 Linked3_V18::xxx() 调用
 *   - 懒加载: 按需加载子模块，不预加载全部
 *   - 向后兼容: 保留原类名直接调用能力
 *
 * 调用示例:
 *   // 逆向拆解
 *   $result = Linked3_V18::reverse_parse($json, 'visual_system');
 *
 *   // 能知约束
 *   $constraint = Linked3_V18::neng_constraint('T1');
 *
 *   // SVG统计
 *   $stats = Linked3_V18::svg_stats('D08');
 *
 *   // 健康检查
 *   $health = Linked3_V18::health_check();
 *
 ...
 * Original file: src/Classes/V18/class-linked3-v18-facade.php
 * Original class: Linked3_V18_Facade
 *
 * @package Linked3\Classes\OS
 */
...
class Linked3_V18 {
    ...
    private static $module_map = [
        ...
        // 集成中心
        'hub'                   => 'Linked3_V18_Integration_Hub_V2',
        // Admin
        'dashboard'             => 'Linked3_V18_Dashboard',
        // API
        'rest_api'              => 'Linked3_V18_Rest_Api',
        'db_schema'             => 'Linked3_V18_Db_Schema',
    ];
    ...
    // ================================================================
    // 集成中心 (Linked3_V18_Integration_Hub_V2)
    // ================================================================
    ...
    public static function register() : void {
        ...
        // 注册REST API
        if (class_exists('\Linked3\Classes\OS\Linked3_V18_Rest_Api') && method_exists('\Linked3\Classes\OS\Linked3_V18_Rest_Api', 'register')) {
            call_user_func(['Linked3_V18_Rest_Api', 'register']);
        }

        // 注册短代码
        if (class_exists('\Linked3\Classes\OS\Linked3_V18_Shortcodes') && method_exists('\Linked3\Classes\OS\Linked3_V18_Shortcodes', 'register')) {
            call_user_func(['Linked3_V18_Shortcodes', 'register']);
        }

        // 注册Widget
        if (class_exists('\Linked3\Classes\OS\Linked3_V18_Widget') && method_exists('\Linked3\Classes\OS\Linked3_V18_Widget', 'register')) {
            call_user_func(['Linked3_V18_Widget', 'register']);
        }

        // 注册Admin面板
        if (class_exists('\Linked3\Classes\OS\Linked3_V18_Dashboard') && method_exists('\Linked3\Classes\OS\Linked3_V18_Dashboard', 'register')) {
            call_user_func(['Linked3_V18_Dashboard', 'register']);
        }

        // 注册DB Schema (激活时创建表)
        if (class_exists('\Linked3\Classes\OS\Linked3_V18_Db_Schema') && method_exists('\Linked3\Classes\OS\Linked3_V18_Db_Schema', 'register')) {
            call_user_func(['Linked3_V18_Db_Schema', 'register']);
        }
    }
