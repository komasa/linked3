...
$seed_count = 0;
$seed_categories = [];
if (class_exists('\Linked3\Classes\Genesis\SeedAdminRender') || class_exists('GenesisSeedCPT')) {
    try {
        ...
        // 按分类统计
        $cats = ['char' => __('角色', 'linked3'), 'brand' => __('品牌', 'linked3'), 'scene' => __('场景', 'linked3'), 'prop' => __('道具', 'linked3'), 'style' => __('画风', 'linked3'), 'soul' => __('灵魂风格', 'linked3')];
...
<h2><?php echo esc_html__('🧬 SEED DNA 库', 'linked3'); ?><span style="font-size:12px;color:#71717A;font-weight:normal;"><?php echo esc_html__('v17.2.0 · 角色/品牌/场景/道具/画风/灵魂', 'linked3'); ?></span></h2>

<div class="notice notice-info inline"><p><strong>SEED DNA:</strong><?php echo esc_html__('视觉生态的基因库。角色DNA(脸型/体型/服装)、品牌DNA、场景DNA、道具DNA、画风DNA、灵魂风格DNA——所有视觉内容(图示/漫画/视频)从这里拉取SEED保持一致性。', 'linked3'); ?></p></div>
...
<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:12px;margin:16px 0;">
    <div style="background:#fff;border:1px solid #f3f4f6;border-radius:8px;padding:14px;text-align:center;">
        ...
        <div style="font-size:11px;color:#71717A;margin-top:2px;"><?php echo esc_html__('SEED 总数', 'linked3'); ?></div>
    </div>
...
<?php if ($seed_count === 0) : ?>
<div class="notice notice-warning inline"><p><?php echo esc_html__('💡 SEED DNA库为空。创建SEED后, 视觉生态的图示/漫画/视频脚本将自动拉取SEED保持角色和画风一致性。', 'linked3'); ?></p></div>
<?php endif; ?>

<!-- 使用说明 -->
<div style="background:#f9fafb;border-radius:8px;padding:14px;margin-top:16px;">
    <h4 style="margin:0 0 8px 0;font-size:13px;"><?php echo esc_html__('📖 SEED DNA 使用流程', 'linked3'); ?></h4>
    <div style="font-size:12px;color:#3F3F46;line-height:1.8;">
        <div><?php echo esc_html__('① 在此创建SEED DNA (角色/品牌/场景/道具/画风/灵魂风格)', 'linked3'); ?></div>
        ...
        <div><?php echo esc_html__('③ 脚本生成时自动拉取SEED DNA, 保持跨分镜的角色和画风一致', 'linked3'); ?></div>
        <div><?php echo esc_html__('④ 生成结果可保存草稿或去发布', 'linked3'); ?></div>
    </div>
</div>
