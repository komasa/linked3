...
= 27.6.22 =
* 致命修复: ChartsRenderer 与 ScriptFactoryTrait 的 $script_type 属性默认值冲突 (E_COMPILE_ERROR)
* 致命修复: ChartsRenderer 缺少 trait 抽象方法 project() 实现 (E_COMPILE_ERROR)
* 致命修复: ChartsFactory compile() 返回类型 : mixed 与 trait 声明 : array 不兼容 (E_COMPILE_ERROR)
* 修复方式: 删除类体属性声明改为构造函数赋值, 补全 project() 方法, 修正返回类型
* 扫描器升级: linked3-ultra-early-scanner v1.1.0 → v1.3.0, 新增 Check 6 (trait 兼容性检查)
* Check 6 检测: (a) trait属性默认值冲突 (b) trait抽象方法未实现 (c) trait抽象方法返回类型不兼容
